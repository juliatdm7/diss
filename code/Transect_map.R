...
ggplot() +
  ...
  geom_sf(data = sites, pch = 16, size = 2, aes(color = as.factor(Current.Boxes)), alpha = 1) + #Transect sites
  theme_bw() +  # Gridlines and latitude as axes
  labs(x="Longitude", y="Latitude") +
  coord_sf(xlim = c(-8, -1), ylim = c(54.6, 59.55)) +
  guides(color=guide_legend(title="Nest boxes"), ) +
  scale_color_manual(values = c("#445690", "#248fc9", "#d5d572"))
ggsave("figures/sitesMap_VI.png")
