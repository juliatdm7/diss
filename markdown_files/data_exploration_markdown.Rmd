...
nestlings <- read.csv("C:/Users/julia/OneDrive - University of Edinburgh/EEB_MSc_UEd/Dissertation/diss/data/NestlingsII.csv")
...
ggplot() +
  ...
  theme(axis.title = element_text(size = 14), 
        axis.text = element_text(size = 12),
        axis.title.x = element_text(margin = unit(c(3, 0, 0, 0), "mm")),
        axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")))  # Here, we can see that between 1 and 3 years of longevity, first egg lay date is earlier. However, after that the trend is more irregular (since we have fewer observations)ç
...
### 8. Correlations between habitat/site variables

```{r, fig.align='center', fig.width=12, fig.height=10}
# I would like to see how different oak-related variables correlate with each other:
...
### 9. Correlations between response variables
...
rv <- blutiphen[,c(11,13,36)]  # selecting first egg lay date, clutch size and number of fledgelings
...
### 10. Correlation between male and female age
...
nestlings <- nestlings %>%
  ...
  dplyr::rename(ringedyear = year) %>%
  dplyr::arrange(ringedyear, site, box)
...
dupes <- age_df %>% 
  dplyr::summarise(n = dplyr::n(), .by = c(year, site, box, sex)) %>% 
  dplyr::filter(n > 1L) 
